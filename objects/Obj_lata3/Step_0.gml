...
if (global.aplastando){
	...
	if(lavado and material_quitado and aplastado==false){
		aplastado = true;
		image_index = 3;
		audio_play_sound(Snd_Right,1,false);
	}else{
...
if(global.clasificando){
	...
	if(aplastado and material_quitado){
		clasificado=true;
		global.clasificados+=1;
		ds_list_add(global.listaPreprocesados, Obj_lata3);
...
if (error >=3){
	error = 0;
	instance_create_layer(2800,600, "Instances", Obj_ErrorLata);
}
